...
class PPOTrainer(BaseRLTrainer):
    ...
    def _init_train(self):
        ...
        self.rollouts = RolloutStorageOracle( #RolloutStorage(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            obs_space,
...
class PPOTrainerObjectRecog(PPOTrainer):
    r"""Trainer class for ObjectRecog
    """
    ...
    def _init_train(self):
        ...
        self.rollouts = RolloutStorageObjRecog(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            obs_space,
            self.policy_action_space,
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.actor_critic = BaselinePolicyObjectRecog(
    ...
    @rank0_only
    def _training_log(
        self, writer, losses: Dict[str, float], prev_time: int = 0, lr_scheduler:Optional = None
    ):
        deltas = {
            k: (
                (v[-1] - v[0]).sum().item()
        ...
        assert total_actions == (total_found_actions + total_forward_actions + 
            total_left_actions + total_right_actions
        )
    ...
    def train(self) -> None:
        ...
        with (
            ...
            while not self.is_done():
                ...
                self._training_log(writer, losses, prev_time, lr_scheduler=lr_scheduler)
...
class PPOTrainerSemantic(PPOTrainer):
    ...
    def _training_log(
        ...
        # model histograms for debugging network learning
        hist_frequency = 100
        if (self.num_steps_done % hist_frequency) == 0:
...
@baseline_registry.register_trainer(name="oracle-map")
class PPOTrainerOracleMap(PPOTrainer):
    r"""Trainer class for Oracle Map agents
    """
    ...
    def _init_train(self):
        ...
        self.rollouts = RolloutStorageOracleMap(
            ppo_cfg.num_steps,
            self.envs.num_envs,
            obs_space,
            self.policy_action_space,
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.actor_critic = BaselinePolicyOracleMap(
            agent_type = self.config.TRAINER_NAME,
            observation_space=self.envs.observation_spaces[0],
            action_space=self.envs.action_spaces[0],
            hidden_size=ppo_cfg.hidden_size,
